...
from loaders.get_data import get_dataset_loader
from loaders.humanml.scripts.motion_process import recover_from_ric
import loaders.humanml.utils.paramUtil as paramUtil
from loaders.humanml.utils.plot_script import plot_3d_motion
import shutil
from loaders.tensors import collate


def render_motions(motion_file, out_path, input_type="text", input_file=None, num_repetitions=3, prompt='A person is ...', dataset='humanml', motion_length=9):
    # args = generate_args()
    # fixseed(args.seed)
    ...
    #     if args.text_prompt != '':
    #         out_path += args.text_prompt.replace(' ', '_').replace('.', '')
    #     elif args.input_text != '':
    #         out_path += os.path.basename(args.input_text).replace('.txt', '').replace(' ', '_').replace('.', '')
    ...
    npy_path = os.path.join(out_path, 'motions.npy')
...
def construct_template_variables(unconstrained):
    row_file_template = 'prompt{:02d}.mp4'
    all_file_template = 'prompts_{:02d}_to_{:02d}.mp4'
    if unconstrained:
        sample_file_template = 'row{:02d}_col{:02d}.mp4'
        sample_print_template = '[{} row #{:02d} column #{:02d} | -> {}]'
        row_file_template = row_file_template.replace('prompt', 'row')
        row_print_template = '[{} row #{:02d} | all columns | -> {}]'
        all_file_template = all_file_template.replace('prompts', 'rows')
        all_print_template = '[rows {:02d} to {:02d} | -> {}]'
    else:
        sample_file_template = 'prompt{:02d}_rep{:02d}.mp4'
        sample_print_template = '["{}" ({:02d}) | Rep #{:02d} | -> {}]'
        row_print_template = '[ "{}" ({:02d}) | all repetitions | -> {}]'
        all_print_template = '[prompts {:02d} to {:02d} | all repetitions | -> {}]'
